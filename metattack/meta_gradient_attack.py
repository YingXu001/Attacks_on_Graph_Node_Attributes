...
class GNNAttack:
    ...
    def __init__(self, adjacency_matrix, attribute_matrix, labels_onehot, hidden_sizes, train_iters=100, gpu_id=None,
        ...
        with self.graph.as_default():
            ...
            self.idx_labeled = tf.keras.Input(shape=(None,), dtype=tf.int32, name="Labeled_Idx")
            self.idx_unlabeled = tf.keras.Input(shape=(None,), dtype=tf.int32, name="Unlabeled_Idx")
            self.idx_attack = tf.keras.Input(shape=(None,), dtype=tf.int32, name="Attack_Idx")
            ...
            for ix, layer_size in enumerate(self.hidden_sizes):
                ...
                weight = tf.Variable(initial_value=w_init(shape=[previous_size, layer_size], dtype=self.dtype), name=f"W_{ix + 1}")
                bias = tf.Variable(initial_value=w_init(shape=[layer_size], dtype=self.dtype), name=f"b_{ix + 1}")
                w_velocity = tf.Variable(initial_value=np.zeros(weight.shape), dtype=self.dtype, name=f"Velocity_{ix + 1}")
                b_velocity = tf.Variable(initial_value=np.zeros(bias.shape), dtype=self.dtype, name=f"b_Velocity_{ix + 1}")
            ...
            output_weight = tf.Variable(initial_value=w_init(shape=[previous_size, self.K], dtype=self.dtype), name=f"W_{len(self.hidden_sizes) + 1}")
            output_bias = tf.Variable(initial_value=w_init(shape=[self.K], dtype=self.dtype), name=f"b_{len(self.hidden_sizes) + 1}")

            output_velocity = tf.Variable(initial_value=np.zeros(output_weight.shape), dtype=self.dtype, name=f"Velocity_{len(self.hidden_sizes) + 1}")
            output_bias_velocity = tf.Variable(initial_value=np.zeros(output_bias.shape), dtype=self.dtype, name=f"b_Velocity_{len(self.hidden_sizes) + 1}")
...
class GNNMetaApprox(GNNAttack):
    ...
    def make_loss(self, ll_constraint=True, ll_cutoff=0.004):
        ...
        with self.graph.as_default():
            ...
            self.adjacency_update = tf.tensor_scatter_nd_add(self.adjacency_changes,
                                                   indices=adj_argmax_combined,
                                                   updates=-2 * tf.gather(
                                                            tf.reshape(self.modified_adjacency, [-1]),
                                                            adj_argmax_combined) + 1)
...
class GNNMeta(GNNAttack):
    ...
    def make_loss(self, ll_constraint=True, ll_cutoff=0.004):
        ...
        with self.graph.as_default():
            ...
            # adj_argmax_combined = tf.expand_dims(adj_argmax_combined, 0)

            indices=adj_argmax_combined
            indices = tf.reshape(indices, [-1, 1])
            updates = -2 * tf.gather(tf.reshape(self.modified_adjacency, [-1]), adj_argmax_combined) + 1


            # print(f"adjacency_changes: {self.adjacency_changes}")
            # print(f"indices: {adj_argmax_combined}")
            # print(f"updates: {updates}")
            # Add the change to the perturbations.
            self.adjacency_meta_update = tf.tensor_scatter_nd_add(self.adjacency_changes,
                                                        indices=indices,
                                                        updates=-2 * tf.gather(
                                                            tf.reshape(self.modified_adjacency, [-1]),
                                                            adj_argmax_combined) + 1)
            ...
            if self.attack_features:
                ...
                self.attribute_meta_update = tf.tensor_scatter_nd_add(self.attribute_changes,
    ...
    def attack(self, perturbations, idx_labeled, idx_attack, initialize=True):
        ...
        with self.graph.as_default():
            ...
            for _it in tqdm(range(perturbations), desc="Perturbing graph"):
                # print(f"idx_attack shape: {idx_attack.shape}")
                # print(f"idx_attack: {idx_attack}")
                # print(f"idx_labeled shape: {idx_labeled.shape}")
                # print(f"idx_labeled: {idx_labeled}")
                idx_attack = idx_attack.reshape(len(idx_attack),1)
                idx_labeled = idx_labeled.reshape(len(idx_labeled),1)
...
class GCNSparse:
    ...
    def __init__(self, adjacency_matrix, attribute_matrix, labels_onehot, hidden_sizes, gpu_id=None,
        ...
        with self.graph.as_default():
            ...
            self.idx = tf.keras.Input(shape=(None,), dtype=tf.int32)
            self.labels_onehot = labels_onehot
            ...
            for ix, layer_size in enumerate(self.hidden_sizes):
                ...
                #                        initializer=w_init())
                weight = tf.Variable(initial_value=w_init(shape=[previous_size, layer_size], dtype=tf.float32), name=f"W_{ix + 1}")
                bias = tf.Variable(initial_value=w_init(shape=[layer_size], dtype=tf.float32), name=f"b_{ix + 1}")
            ...
            #                              initializer=w_init())
            weight_final = tf.Variable(initial_value=w_init(shape=[previous_size, self.K], dtype=tf.float32), name=f"W_{len(hidden_sizes) + 1}")
            bias_final = tf.Variable(initial_value=w_init(shape=[self.K], dtype=tf.float32), name=f"b_{len(hidden_sizes) + 1}")
    ...
    def train(self, idx_train, n_iters=200, initialize=True, display=True):
        ...
        idx_train = idx_train.reshape(len(idx_train),1)
        # print(f"new idx_train shape: {idx_train.shape}")
        with self.graph.as_default():
            if initialize:
                self.session.run(tf.compat.v1.global_variables_initializer())
